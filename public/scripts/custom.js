...
function deletefun(id,text){
  ...
  //prevent forms double submit
  var fdsubmit = document.querySelectorAll('form');
  for(i=0 ; i < fdsubmit.length ; i++ ){
      fdsubmit[i].addEventListener('submit', e => {
        if (fdsubmit[i].data('submitted') === true) {
            ...
            fdsubmit[i].data('submitted', true);
            fdsubmit[i].find("button").append('<span class="spinner-border spinner-border-sm ml-2" role="status" aria-hidden="true"></span>');
...
function markComplete(id){
        ...
        if (input.checked == true){
            document.getElementById('status'+id).value = "completed";
          }
           if (input.checked == false) {
            document.getElementById('status'+id).value = "not completed";
          }

          document.getElementById('markform'+id).submit();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var pwaName = "Alaa Projects"; //Local Storage Names for PWA
    var pwaRemind = 1; //Days to re-remind to add to home
    var pwaNoCache = false; //Requires server and HTTPS/SSL. Will clear cache with each visit

    //Setting Service Worker Locations scope = folder | location = service worker js location
    var pwaScope = "/haririapp/public";
    var pwaLocation = "/haririapp/public/_service-worker.js";
